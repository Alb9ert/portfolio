...
function ProjectDetail() {
  ...
  const handleBackClick = () => {
    ...
    return (
    <div className="pt-28 sm:pt-25 bg-[#ececec] overflow-hidden flex flex-col relative">
      

      {/* Image Gallery at the top */}
      <div className="w-full flex justify-center items-center flex-col px-4 sm:px-6 md:px-8">
         <motion.div 
          ref={buttonsRef}
          className="w-full md:w-3/4 lg:w-1/2 py-4 sm:py-6 flex justify-between items-center"
          initial={{
            opacity: 0,
            y: 20
          }}
        ...
        >
           <motion.button 
             className="cursor-pointer px-4 py-2 bg-black text-white rounded relative overflow-hidden text-sm sm:text-base"
           ...
           <div className="flex gap-2 sm:gap-4">
             {project?.githubLink && (
               <motion.button 
                 className="cursor-pointer px-4 py-2 bg-black text-white rounded flex items-center gap-2 relative overflow-hidden text-sm sm:text-base"
        ...
        <motion.div 
          ref={titleRef}
          className="w-full md:w-3/4 lg:w-1/2 flex flex-col sm:flex-row justify-between mt-6 sm:mt-8 pb-2 border-b border-gray-400 gap-2 sm:gap-4"
          initial={{
            opacity: 0,
            y: 20
          }}
         ...
         >
           <p className="mt-4 sm:mt-5 text-base sm:text-lg md:text-xl font-base">{project?.details}</p>
         </motion.div>
          <motion.div 
           ref={techStackRef}
           className="w-full md:w-3/4 lg:w-1/2 flex flex-col md:flex-row justify-between mt-8 sm:mt-10 md:mt-12 gap-8 md:gap-12 lg:gap-16 pb-32 sm:pb-40 md:pb-60"
          initial={{
            opacity: 0,
            y: 20
          }}
        ...
        >
           <div className="flex-1">
              <h3 className="text-xs sm:text-sm uppercase tracking-wider opacity-50 mb-4 sm:mb-6">TECHNOLOGIES</h3>
               <div className="flex flex-wrap gap-2">
                {project?.techStack.map((tech, index) => (
                  <span key={index} className="px-2 sm:px-3 py-1 bg-white text-xs sm:text-sm font-medium border border-gray-200">
                     {tech}
                   </span>
                 ))}
               </div>
           ...
           >
              <h3 className="text-xs sm:text-sm uppercase tracking-wider opacity-50 mb-4 sm:mb-6">KEY FEATURES</h3>
              <div className="flex flex-wrap gap-2">
